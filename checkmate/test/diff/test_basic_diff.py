...
class TestDiffing(ProjectBasedTest):
    ...
    def test_simple_diff(self):
        ...
        file_revision_1 = FileRevision(
            {'path': 'foo.py', 'hash': '1:foo.py', 'language': 'python'})

        issue_class_1 = IssueClass(
            {'code': 'wildcard', 'analyzer': 'example', 'severity': 2})
        project_issue_class_1 = ProjectIssueClass({'project': self.project,
                                                   'issue_class': issue_class_1,
                                                   'enabled': True})
        issue_1 = Issue({'code': 'wildcard', 'analyzer': 'example',
                        'fingerprint': '1', 'project': self.project})
        ...
        file_revision_2 = FileRevision(
            {'path': 'bar.py', 'hash': '2:bar.py', 'language': 'python'})
        snapshot_2.file_revisions = [file_revision_1, file_revision_2]
        issue_class_2 = IssueClass(
            {'code': 'format', 'analyzer': 'example', 'severity': 1})
        project_issue_class_2 = ProjectIssueClass({'project': self.project,
                                                   'issue_class': issue_class_2,
                                                   'enabled': True})
        issue_2 = Issue({'code': 'format', 'analyzer': 'example',
                        'fingerprint': '1', 'project': self.project})
        ...
        diff, diff_file_revisions, diff_issue_occurrences = environment.diff_snapshots(
        ...
        file_revision_3 = FileRevision(
            {'path': 'bar.py', 'hash': '3:bar.py', 'language': 'python'})
        ...
        assert summarized_issues['added']['']['python']['example']['format'] == [
            1, 1]
