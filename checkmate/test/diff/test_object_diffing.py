...
class TestObjectDiffing(TestCase):
    ...
    def test_issue_occurrence_diffing(self):
        ...
        issue_1 = Issue(
            {'analyzer': 'test', 'code': 'test1', 'fingerprint': 'foo'})
        issue_2 = Issue(
            {'analyzer': 'test', 'code': 'test2', 'fingerprint': 'bar'})
        ...
        assert issue_occurrence_key(
            issue_occurrence_1) == issue_occurrence_key(issue_occurrence_2)
        assert issue_occurrence_key(
            issue_occurrence_3) != issue_occurrence_key(issue_occurrence_1)
        assert issue_occurrence_comparator(
            issue_occurrence_1, issue_occurrence_2) == 0
        assert issue_occurrence_comparator(
            issue_occurrence_1, issue_occurrence_4) == -1
        assert issue_occurrence_comparator(
            issue_occurrence_1, issue_occurrence_3) == -1

        result = diff_objects([issue_occurrence_1, issue_occurrence_3],
                              [issue_occurrence_1, issue_occurrence_2,
                                  issue_occurrence_4],
                              key=issue_occurrence_key,
                              comparator=issue_occurrence_comparator)

        for key, issue_occurrences in list(result.items()):
            if key == 'added':
    ...
    def test_file_revision_diffing(self):
        ...
        assert file_revision_key(
            file_revision_1) == file_revision_key(file_revision_2)
        assert file_revision_key(
            file_revision_1) != file_revision_key(file_revision_3)
        ...
        for key, file_revisions in list(result.items()):
            if key == 'added':
